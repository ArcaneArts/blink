...
import 'package:blink/util/nst.dart';
...
class MobileP2PNode {
  ...
  Future<void> _performCleanup() async {
    ...
    if (tableSize > maxRoutingTableSize) {
      print(
        '[$nodeId] Routing table size ($tableSize) exceeds limit ($maxRoutingTableSize)',
      );
  ...
  Future<List<AddrInfo>> findContentProviders(
    ...
    print(
      '[$nodeId] Looking for providers of content: $contentId (max: $maxProviders)',
    );
    ...
    try {
      ...
      await for (final provider in stream.timeout(
        timeout ?? Duration(seconds: 30),
      )) {
        providers.add(provider);
        print(
          '[$nodeId] Found provider: ${provider.id.toBase58().substring(0, 8)}',
        );
  ...
  Future<List<AddrInfo>> findServiceProviders(
    ...
    print(
      '[$nodeId] Looking for service providers: $serviceName (max: $maxPeers)',
    );
    ...
    try {
      ...
      await for (final peer in stream.timeout(
        timeout ?? Duration(seconds: 30),
      )) {
        providers.add(peer);
        print(
          '[$nodeId] Found service provider: ${peer.id.toBase58().substring(0, 8)}',
        );
    ...
    print(
      '[$nodeId] Found ${providers.length} providers for service: $serviceName',
    );
    return providers;
  }
  ...
  Future<void> printNetworkStats() async {
    ...
    print(
      '  - Background refresh: ${stats.isRefreshActive ? "Active" : "Inactive"}',
    );
    print(
      '  - Memory cleanup: ${stats.isCleanupActive ? "Active" : "Inactive"}',
    );
  }
